...
def bfs(pos):
    ...
    while q: # queue가 비워질때까지
        ...
        for i, j in zip(dx, dy): #상,하,좌,우로 탐색
            ...
            if visi[nx][ny]=='-' or visi[nx][ny]==-1 or visi[nx][ny]!=0: continue # 벽과 비활성 바이러스는 고려하지 않는다.
            if visi[nx][ny]=='*':
...
for pos in positions:
    result=bfs(pos)
    if result!=-1:
        answer=min(answer, result) # 모든 경우의 수에 대해 bfs를 사용하기

print(-1 if answer==math.inf else answer) # answer이 초기값 그대로라면, 옮길 수 있는게 없으므로 -1 반환 / 아니라면 최소값 반환